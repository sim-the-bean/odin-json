...
read_from_path :: proc(path: string) -> (value: values.Value, ok: bool) {
    bytes: []byte;
...
read_from_string :: proc(text: string) -> (value: values.Value, ok: bool) {
    i := 0;
...
@private
parse_element :: proc(text: string, pos: ^int) -> (value: values.Value, ok: bool) {
    parse_whitespace(text, pos);
...
@private
parse_elements :: proc(text: string, pos: ^int) -> (array: values.Array, ok: bool) {
    array = make(values.Array);
    for {
        value: values.Value;
...
@private
parse_member :: proc(text: string, pos: ^int) -> (key: values.String, value: values.Value, ok: bool) {
    parse_whitespace(text, pos);
...
@private
parse_members :: proc(text: string, pos: ^int) -> (object: values.Object, ok: bool) {
    object = make(values.Object);
    for {
        key: values.String;
        value: values.Value;
...
@private
parse_value :: proc(text: string, pos: ^int) -> (value: values.Value, ok: bool) {
    r: rune;
    if r, ok = peek_rune(text, pos^); !ok do return;
    switch r {
    case '{':
...
@private
parse_object :: proc(text: string, pos: ^int) -> (value: values.Object, ok: bool) {
    r: rune;
...
@private
parse_array :: proc(text: string, pos: ^int) -> (value: values.Array, ok: bool) {
    r: rune;
    if r, ok = next_rune(text, pos); !ok || r != '[' {
        log.errorf("odin-json: unexpected character at %v, expected `[`", pos^ - 1);
        ok = false;
...
@private
parse_string :: proc(text: string, pos: ^int) -> (value: values.String, ok: bool) {
    r: rune;
    if r, ok = next_rune(text, pos); !ok || r != '"' {
        log.errorf("odin-json: unexpected character at %v, expected `\"`", pos^ - 1);
        ok = false;
    ...
    if r, ok = peek_rune(text, pos^); !ok {
        ...
        str = values.String(string(make([]u8, 0)));
    } else {
...
@private
parse_identifier :: proc(text: string, pos: ^int) -> (value: values.Value, ok: bool) {
    r: rune;
    if r, ok = peek_rune(text, pos^); !ok do return;
    switch r {
    case 'n':
    ...
    case 't':
        return parse_identifier_priv(text, pos, "true", values.Bool(true));
    case 'f':
        return parse_identifier_priv(text, pos, "false", values.Bool(false));
    case:
...
@private
parse_identifier_priv :: proc(text: string, pos: ^int, ident: string, val: values.Value) -> (value: values.Value, ok: bool) {
    for r0 in ident {
        r1: rune;
...
@private
parse_number :: proc(text: string, pos: ^int) -> (number: values.Number, ok: bool) {
    ...
    number = values.Number(float);
    ok = true;
    return;
}

@private
parse_characters :: proc(text: string, pos: ^int) -> (str: values.String, ok: bool) {
    runes := make([dynamic]rune);
    defer delete(runes);
    r: rune;
    loop: for {
    ...
    ok = true;
    str = values.String(utf8.runes_to_string(runes[:]));
    return;
}
